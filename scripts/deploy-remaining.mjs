// deploy-remaining.mjs — Deploy OTCEscrow to remaining 20 EVM chains
// Ethereum already deployed. This script has proper try/catch per chain.
...
// Remaining 20 chains (Ethereum already done)
const chains = [
...
const deployed = [];
const failed = [];
const txSent = []; // Track TXs sent but not yet confirmed

console.log("═".repeat(60));
console.log("  OTCEscrow — Deploy to 20 remaining EVM Chains");
console.log("  (Ethereum already done: 0x8DCB419C1479b70B2BdB57cf5482cbf33E5E7f40)");
console.log("═".repeat(60));
console.log("");
...
for (let i = 0; i < chains.length; i++) {
    ...
    try {
        ...
        if (balance === 0n) {
            ...
            failed.push({ name: c.name, chainId: c.chainId, reason: "no funds" });
            console.log("");
            continue;
        }
        ...
        try {
            ...
            console.log(`    ⏳ TX sent but confirmation timed out — TX: ${tx.hash}`);
            txSent.push({ name: c.name, chainId: c.chainId, tx: tx.hash });
        }
    } catch (err) {
        const msg = err.message?.substring(0, 150) || String(err);
        console.log(`    ❌ FAILED: ${msg}`);
        failed.push({ name: c.name, chainId: c.chainId, reason: msg });
    }
    console.log("");
}
...
console.log("═".repeat(60));
console.log("");
console.log(`  Ethereum: 0x8DCB419C1479b70B2BdB57cf5482cbf33E5E7f40 ✅`);
console.log("");
...
if (txSent.length > 0) {
    console.log("");
    console.log(`⏳ TX sent but needs manual check (${txSent.length}):`);
    for (const t of txSent) {
        console.log(`   ${t.name.padEnd(15)} TX: ${t.tx}`);
    }
}
if (failed.length > 0) {
    console.log("");
